...
function Address() {
  ...
  async function fetchcontactaddress() {
    try {
      const res = await FetchContact();
      console.log('resget', res);
      setcontactoffice(res);
    } catch (error) {
    ...
    <div className="max-w-7xl mx-auto mt-10 px-2 md:px-8 gap-8">
      ...
      <div className="flex flex-col mx-auto md:flex-row gap-6 px-1 py-8">
        {contactoffice.map((office) => (
          <div 
            key={office._id} 
            className="flex flex-col bg-white shadow-lg rounded-lg overflow-hidden md:w-1/2"
          >
            {/* Image Section */}
            ...
            <div className="p-4 text-left">
              ...
              <p className="text-sm md:text-lg text-gray-600 leading-relaxed">
                ...
                {office.country} - {office.pin}
              </p>

              <button className="bg-[#FD5D14] text-white px-3 py-2 mt-4 rounded-md">
                <a 
                  className="flex gap-1 items-center" 
                  href={office.findonmap}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg 
                    className="w-[24px] size-6" 
